...
// No public bucket policy — all client access uses presigned URLs
// generated by the Go backend via GetPresignedURL/GetSegmentPresignedURL.
...
func (m *Minio) makeSureBucketExists(ctx context.Context) error {
	...
	if err != nil {
		exists, existsErr := m.client.BucketExists(ctx, bucketName)
		if existsErr != nil {
			return fmt.Errorf("cannot check if bucket exists: %w", existsErr)
		}
...
func (m *Minio) CloseRecordingSession(ctx context.Context, recorderID, sessionID uuid.UUID) error {
	...
	if chunkCopy != nil {
		...
		if chunkCopy.streaming != nil {
			totalBytes += chunkCopy.streaming.totalBytes
		}
